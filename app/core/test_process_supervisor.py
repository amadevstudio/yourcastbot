...
def test_children_share_supervisor_process_group():
    tmpdir = tempfile.mkdtemp(prefix="yourcast_supervisor_pgid_")
    ...
    try:
        ...
        assert os.getpgid(proc.pid) == os.getpgid(os.getpid()), (
            "child must stay in the supervisor process group so "
            "supervisord killasgroup tears down the tree")
        print("ok  child pgid matches supervisor", os.getpgid(proc.pid))
    finally:
        supervisor.terminate_all(timeout=5)
...
def main():
    ...
    test_children_share_supervisor_process_group()
